...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a kaishen address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no kaishen: URI
...
{
    // Convert kaishen:// to kaishen:
    //
    //    Cannot handle this later, because kaishen:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open kaishen.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "kaishen.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for kaishen.lnk
...
{
    return GetAutostartDir() / "kaishen.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a kaishen.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=kaishen\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the kaishen app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add kaishen app to startup item list
