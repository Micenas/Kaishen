...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid kaishen address: ") + input.get_str());
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    const CCoins* existinkaishens = view.AccessCoins(hashTx);
    bool fHaveMempool = mempool.exists(hashTx);
    bool fHaveChain = existinkaishens && existinkaishens->nHeight < 1000000000;
