...
int nAnonymizegcoinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "gcoin" is a composite category enabling all gcoin-related debug output
            if (ptrCategory->count(string("gcoin"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty gcoin.conf if it does not exist
